#!/usr/bin/env python3
"""Extract route profitability table from raw PDF/image/text into interim CSV files."""
...
class ExtractionError(RuntimeError):
    """Raised when a row cannot be interpreted as expected route-profit table data."""


def normalize_numeric(value: str) -> str:
    """Normalize Japanese numeric strings to machine-readable form."""
    text = str(value).strip()
    text = text.translate(str.maketrans({"，": ",", "．": ".", "－": "-", "ー": "-", "―": "-"}))
    text = text.replace("円", "").replace("人", "").replace(" ", "").replace("\u3000", "")
    text = text.replace(",", "")
    if text.startswith("▲"):
        text = f"-{text[1:]}"
    text = text.replace("−", "-")
...
def normalize_profit(value: str) -> str:
    """Normalize 差引損益 field; convert ▲ notation to negative numeric representation."""
...
def extract_text(path: Path) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install dependencies or provide a .txt/.csv source for parsing."
        ) from exc
...
def detect_rows(text: str) -> Iterable[list[str]]:
    """Yield table-like rows separated by comma/tab/multi-space with minimum 7 columns."""
...
def validate_numeric(field_name: str, value: str, allow_float: bool) -> str:
    pattern = r"^-?\d+(?:\.\d+)?$" if allow_float else r"^-?\d+$"
    if not re.match(pattern, value):
        raise ExtractionError(f"Invalid numeric format for {field_name}: {value}")
    return value


def parse_rows(text: str, fiscal_year: int, strict: bool = True) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    for parts in detect_rows(text):
        if parts[0] in {"系統", "route_id"}:
            continue

        route_id = normalize_numeric(parts[0])
        ...
        try:
            route_id = validate_numeric("系統", route_id, allow_float=False)
            revenue = validate_numeric("収入", revenue, allow_float=False)
            cost = validate_numeric("費用", cost, allow_float=False)
            profit = validate_numeric("差引損益", profit, allow_float=False)
            operating_ratio = validate_numeric("営業係数", operating_ratio, allow_float=True)
            daily_passengers = validate_numeric("1日当たり輸送人員", daily_passengers, allow_float=False)
...
def main() -> None:
    ...
    parser.add_argument("--no-strict", action="store_true", help="Skip malformed rows instead of failing")
    ...
    for source_str in args.inputs:
        ...
        rows = parse_rows(text, args.fiscal_year, strict=strict)
